function novelCard(novel) {
  return `<article class="novel-card">
    ...
    <div class="novel-body">
      <div class="tags"><span class="tag">${NK.escapeHtml(novel.category)}</span><span class="tag">${NK.escapeHtml(novel.status)}</span></div>
      <h3>${NK.escapeHtml(novel.title)}</h3>
      <p class="novel-meta">✍️ ${NK.escapeHtml(novel.author)} · 👁️ ${Number(novel.reads).toLocaleString('ar')} · ⭐ ${novel.rating}</p>
